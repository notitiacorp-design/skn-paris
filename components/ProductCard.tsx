...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <div className="group cursor-pointer relative overflow-hidden border border-[#121212]/5 hover:border-[#121212]/10 transition-all duration-500">
      ...
      <div className="h-72 bg-gradient-to-b from-[#f0ece2] to-[#d8d4c8] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="p-8 bg-[#F4F1EA] group-hover:bg-[#121212] transition-colors duration-500">
        <p className="text-xs tracking-[0.3em] text-[#C7FF2E] mb-2">{product.num}</p>
        ...
        <div className="flex items-center justify-between">
          <span className="text-lg font-medium group-hover:text-[#C7FF2E] transition-colors">{product.price}</span>
          <button className="text-xs tracking-[0.15em] uppercase py-2 px-4 border border-[#121212]/10 group-hover:border-[#C7FF2E] group-hover:text-[#C7FF2E] transition-all opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0">
            ADD TO BAG →
          </button>
        </div>
      </div>
